...
class Block extends EventEmitter {
	constructor(board, config) {
		...
		if (this.config.type === 'wire' || this.config.type === 'calc') {
			this.rotatedPlugs = this.config.io.plugs.map(this.rotatedDirection);
		} else if (this.config.type === 'wireF') {
			this.rotatedFlow = {};
			const directions = ['top', 'right', 'bottom', 'left'];
			directions.forEach((direction) => {
				rotatedFlow[this.rotatedDirection(direction)] =
					this.config.io.flow[direction].map(this.rotatedDirection);
			});
		}
	}
		...
		switch (this.config.type) {
			...
			case 'wireF': {
				for (const [source, queue] of this.inputQueues.entries()) {
					...
					if (this.config.io.rotatedFlow.includes(source)) {
						const destinations = this.config.io.rotatedFlow[source];
						const data = queue.shift();
