...
class Block extends EventEmitter {
	...
	step() {
		if (this.config.type === 'empty') {
			...
			for (const source in this.inputQueues) {
			...
			if (sources.every((source) => this.inputQueues[source].length > 0)) {
				...
				this.outputQueues[destination].push(outData);
			...
			if (sources.every((source) => this.inputQueues[source].length > 0)) {
				...
				this.outputQueues[destination].push(data);
	...
	pass() {
		for (const direction in this.outputQueues) {
