...
class Game {
	...
	saveStage() {
		const board = this.currentStage.board.serialize();
	...
	loadStage(key) {
		...
		const stageIndex = property.stageIndex;
		const board = property.board;
		...
		this.currentStage = new Stage(this, this.configs[stageIndex]);
		this.currentStage.makeBoard(board);
