...
class Board extends EventEmitter {
	constructor(config) {
		...
		if (typeof config.inputX === "number") this.inputBlockX = [config.inputX];
		else this.inputBlockX = config.inputX;
		this.inputBlockY = 0;
	...
	input(value) {
		const nvarr = (typeof value === "number") ? [value] : value;
		
		const inputData = nvarr.map( (nv,i) => {
			const nd = new Data(this, nv);
			this.inputBlock[i].input('top', nd);
			return nd;
		});
