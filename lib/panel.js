...
class Panel {
	...
	addItem(itemName, count) {
		...
		//this.parts.forEach((obj, index) => {	// search the picked item
		for (let index = 0; index < this.parts.length; index++) {
			let obj = this.parts[index];
			const item = obj.name;
			if (item === itemName) {
				if (obj.count === null) return;	// null means infinity
	...
	setInventory(index, count) {
		if (this.parts[index].count === null) return;
		if (count <= 0) {
	...
	update() {
		...
		this.parts.forEach((obj) => {
			// if (!uniqueParts.has(obj.name)) {
			// 	uniqueParts.set(obj.name, 0);
			// }
			// uniqueParts.set(obj.name, uniqueParts.get(name) + 1);
