...
class Stage {
	...
	executeCase() {
		...
		const inputDataElement = new DataElement(this.boardElement.$board, inputData, this.board.inputBlock.top);
		inputData.element = inputDataElement;

		this.board.step();
		Promise.all(this.boardElement.animations).then(() => {
			this.boardElement.animations = [];
			this.board.pass();
			return Promise.all(this.boardElement.animations);
		}).then(() => {
			this.boardElement.animations = [];
			this.board.step();
			return Promise.all(this.boardElement.animations);
