...
// TODO: 消す
const DataElement = require('./data-component');
...
class Stage {
	...
	executeCase() {
		this.boardComponent.input(this.config.input[this.caseIndex]);
		const inputs = this.board.input(this.config.input[this.caseIndex]);
		inputs.forEach((input, index) => {
			input.forEach((value) => {
				new DataElement(this.boardComponent, value, this.board.inputBlock[index].top);
			});
		});
	...
	clockUp() {
		...
		if (this.board.status === 'executing') {
			Promise.all(this.boardComponent.animations).then(() => {
