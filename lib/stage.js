...
class Stage {
	constructor({config, onClickExit, onAnswer}) {
		...
		this.boardComponentProps = {
			...
			isForced: false,
			moveStatus: 'none',
			finishSelect: this.handleFinishSelect,
			cancelMove: this.handleCancelMove,
			finishMove: this.handleFinishMove,
		};
		...
		this.$stage.find('button.move').click(() => {
			if (this.boardComponentProps.status !== 'executing') {
				if (this.boardComponentProps.moveStatus === 'none') {
					this.boardComponentProps.moveStatus = 'select';
				} else {
					this.boardComponentProps.moveStatus = 'none';
				}
				console.log("move is now ", this.boardComponentProps.moveStatus);
			}
	...
	handleFinishSelect = () => {
		this.boardComponentProps.moveStatus = 'move';
		this.renderBoardComponent();
	}
	...
	handleFinishMove = (selectStart, selectEnd, deltaX, deltaY) => {
		...
		for (let i = left; i <= right; i++) {
			for (let j = top; j <= bottom; j++) {
				save.push(this.boardComponent.getBlock(i, j).config.name);
				this.takeAndPlace(i, j, 'empty');
			}
		}
		for (let i = left + deltaX; i <= right + deltaX; i++) {
			for (let j = top + deltaY; j <= bottom + deltaY; j++) {
				const blockName = save.shift();
				if (0 <= i && i < this.config.width && 0 <= j && j < this.config.height) {
					this.takeAndPlace(i, j, 'empty');
