...
class Stage {
	constructor({config, onClickExit, onAnswer}) {
		...
		$(window).mousedown(() => {
			this.boardComponentProps.isMouseDown = true;
			this.renderBoardComponent();
			return true;
		});

		$(window).mouseup(() => {
			this.boardComponentProps.isMouseDown = false;
			this.renderBoardComponent();
			return true;
		});
		...
		this.$stage.find('button.move').click(() => {
			if (this.boardComponentProps.status === 'stop') {
				if (!this.boardComponentProps.isMovingMode) {
					this.boardComponentProps.isMovingMode = true;
				} else {
