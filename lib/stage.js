...
class Stage {
	constructor(game, config) {
		...
		const boardWidth = parseFloat(this.$stage.find('.board-area').css('width')) - 50;
		const boardHeight = parseFloat(this.$stage.find('.board-area').css('height')) - 70;
		...
		this.$stage.find('.board .block').css({"height": `${this.blockSize}px`})
		this.$stage.find('.board .block').css({"width": `${this.blockSize}px`})
		this.$stage.find('.board .block').css({"background-size": "contain"})
