...
class Stage {
	constructor(game, config) {
		...
		this.$stage.find('.inputs .input').each((index, input) => {
			const $span = $(input).find('.input-span');
			const inputWidth = $(input).outerWidth();
			const spanWidth = $span.width();
			if (inputWidth < spanWidth) {
				$span.css({'font-size': `${inputWidth / spanWidth * 100 - 2}%`});
			}
		});
		...
		this.$stage.find('.outputs .output').each((index, output) => {
			const $span = $(output).find('.output-span');
			const outputWidth = $(output).outerWidth();
			const spanWidth = $span.width();
			if (outputWidth < spanWidth) {
				$span.css({'font-size': `${outputWidth / spanWidth * 100 - 2}%`});
			}
		});
	...
	executeCase() {
		...
		inputData.forEach((d, i) => {
			new DataElement(this.boardElement, d, this.board.inputBlock[i].top);
		});
