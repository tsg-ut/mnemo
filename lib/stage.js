...
class Stage {
	constructor(game, config) {
		...
		this.$stage.find('.inputs').empty().append(config.input.map((input) => {
			...
			if (inputString.length > 15) inputString = inputString.substring(0,13) + '...';
	...
	executeCase() {
		...
		inputData.forEach((d, i) => {
			d.forEach((e) => {
				new DataElement(this.boardElement, e, this.board.inputBlock[i].top);
			});
		});
