...
const {translateDateFromUnixTime} = require('./util');
...
class Stage {
	...
	saveStage() {
		const boardData = this.board.boardData;
	...
	loadStage(stageName, timestamp) {
		...
		assert(stageName === this.config.name);
	...
	getSaveEntries(stageName) {
		const item = localStorage.getItem('boardData');
		if (item === null) {
			return [];
		}
		const properties = JSON.parse(item);
		return properties.filter((prop) => (stageName === prop.stageName));
	}

	removeSaveEntry(stageName, timestamp) {
		const item = localStorage.getItem('boardData');
		let properties = [];
		if (item !== null) {
			properties = JSON.parse(localStorage.getItem('boardData'));
	...
	updateSaveEntries() {
		const entries = this.getSaveEntries(this.config.name);
		...
		this.$savePanel.find('.timestamp').each((index, element) => {
			...
			$timestampButton.click(() => {
				this.loadStage(entry.stageName, entry.timestamp);
				this.updateSaveEntries();
			});
		});
		this.$savePanel.find('.delete').each((index, element) => {
			...
			const entry = entries[index];
			$delButton.click(() => {
				this.removeSaveEntry(entry.stageName, entry.timestamp);
				this.updateSaveEntries();
			});
		});
	}
