...
class Stage {
	constructor(game, config) {
		...
		this.$savePanel = this.$stage.siblings('.save-panel-layer').find('.save-panel');
		this.updateStyles();
	...
	updateSaveEntries() {
		...
		this.$savePanel.find('.timestamp').each((index, element) => {
			...
			$timestampButton.click(() => {
				this.game.loadStage(entry.stageName, entry.timestamp);
				this.updateSaveEntries();
			});
		});
		this.$savePanel.find('.delete').each((index, element) => {
			...
			const entry = entries[index];
			$delButton.click(() => {
				this.game.removeSaveEntry(entry.stageName, entry.timestamp);
				this.updateSaveEntries();
			});
		});
	}
